...
const SideBar = () => {
  ...
  return (
    <Div className=" fixed hidden lg:flex flex-col justify-between w-270wd xl:!px-4 pb-4 pt-24 -mt-1  h-screen z-[35] ">
      <div className="flex flex-col gap-2">
          ...
          {tabs?.map((tab, index) => (
            <Link href={tab?.link}>
              <IconButton
                ...
                className={` ${
                  tab.title == activeTab
                    ? "dark:bg-dark_bg_grey bg-light_bg_grey"
                    : ""
                }`}
        ...
        <div className="flex flex-col gap-3 ">
          ...
          <div className="flex flex-col gap-1 border dark:text-dark_primary_label dark:border-neutral-800 border-neutral-200 rounded-2xl px-4 py-2.5 ">
            <H6
              className={
                "mb-1 dark:!text-dark_secondary_label !text-light_secondary_label"
              }
            >
              马上登录即可
            </H6>
      ...
      <div className="mb-0">
        <DropdownMenu>
          <IconButton icon={"fluent:navigation-24-filled"} className={"w-full"}>
            更多{" "}
          </IconButton>
        </DropdownMenu>
      </div>
