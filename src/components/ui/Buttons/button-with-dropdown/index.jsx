...
const HoverDropdownButton = ({ className, children, onClick, options ,dropdownClasses }) => {
    ...
    return (
        <div className='relative z-50 h-12' onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} >
            ...
            {hover && (
                <div className={`absolute top-11 flex flex-col w-44 max-w-56 dark:bg-custom_black bg-white p-1 drop-shadow-xl rounded-xl  ${hover ? 'flex' : 'hidden'} ${dropdownClasses}`} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
                    {
                        options?.map((opt) => (
